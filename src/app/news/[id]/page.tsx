...
export default function NewsDetailPage() {
  ...
  // === FETCH BERITA UTAMA dari Supabase via API ===
  useEffect(() => {
    ...
    fetch(`/api/analyze-news/berita/${id}`)
      .then((res) => {
  ...
  // === FETCH BERITA RELATED (ambil beberapa artikel lain) ===
  useEffect(() => {
  ...
  if (!loading && notFound) {
    return (
      <main
        className={`${isDarkMode ? "dark" : ""} relative min-h-screen w-full bg-gray-50 dark:bg-[#05051a] flex items-center justify-center transition-colors duration-500`}
      >
        <div className="relative z-10 text-center px-4">
          <p className="text-[80px] font-black text-gray-200 dark:text-white/[0.04] leading-none mb-4 select-none">
            404
          </p>
          ...
          <Link
            href="/"
            className="inline-flex items-center gap-2 bg-gray-900 dark:bg-white text-white dark:text-black px-6 py-3 rounded-xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-blue-600 transition-all duration-300"
          >
            ← Kembali ke Beranda
          </Link>
        </div>
  ...
  return (
    <main
      className={`${isDarkMode ? "dark" : ""} relative min-h-screen w-full bg-gray-50 dark:bg-[#05051a] text-gray-900 dark:text-white transition-colors duration-500`}
    >
      {/* NAVBAR */}
      <nav className="sticky top-0 z-50 border-b border-gray-200 dark:border-white/[0.06] bg-white/80 dark:bg-[#05051a]/80 backdrop-blur-2xl transition-colors duration-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link
            href="/"
            className="flex items-center gap-3 text-[11px] font-black tracking-[0.4em] uppercase text-gray-900 dark:text-white/80 hover:text-blue-600 dark:hover:text-white transition-colors duration-200"
          >
          ...
          <div className="flex items-center gap-5">
              ...
              <div className="hidden sm:flex items-center gap-2">
                {news.aiLoading ? (
                  <>
                    ...
                    <span className="text-[10px] text-amber-600 dark:text-amber-400/70 uppercase tracking-wider font-bold">
                      AI menganalisis…
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-[10px] text-emerald-600 dark:text-emerald-400/70 uppercase tracking-wider font-bold">
                      Analisis Selesai
                    </span>
                  </>
                )}
            ...
            <button
              onClick={() => router.back()}
              className="flex items-center gap-1.5 text-[11px] text-gray-500 dark:text-white/40 hover:text-gray-900 dark:hover:text-white transition-colors duration-200 uppercase tracking-wider font-bold ml-2"
            >
        ...
        {!loading && news && relatedNews.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <Link
                href="/"
                className="text-[11px] text-gray-500 dark:text-white/40 hover:text-blue-600 dark:hover:text-white transition-colors uppercase tracking-widest font-bold"
              >
                Lihat Semua →
              </Link>
            </div>
    ...
    >
      ...
      >
        <div className="p-5">
          ...
          <p className="text-[11px] text-gray-500 dark:text-white/40 mt-3 font-semibold">
            {item.time}
          </p>
        </div>
      </Link>
    ...
    <div className="rounded-[1.5rem] border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-[#070716]/90 backdrop-blur-xl p-6 md:p-8 relative overflow-hidden flex flex-col shadow-xl shadow-gray-200/50 dark:shadow-[0_0_40px_rgba(0,16,245,0.05)] transition-colors duration-500">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[13px] font-black uppercase tracking-[0.2em] text-gray-900 dark:text-white/90">
            AI Insight
          </span>
        </div>
