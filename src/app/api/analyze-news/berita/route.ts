...
export async function GET(request: NextRequest) {
    ...
    const sentSet = new Set<number>(
        (sentRows || []).map((r: any) => r.id_cluster).filter((x: any): x is number => x != null),
    );
    const sektorSet = new Set<number>(
        (sektorRows || []).map((r: any) => r.id_cluster).filter((x: any): x is number => x != null),
    );
    // Irisan: cluster yang punya sentimen DAN sektor
    const analyzedClusterIds = [...sentSet].filter((id: number) => sektorSet.has(id));
    ...
    if (isSearch) {
        ...
        const orStr = terms
            .map((t: string) => `judul.ilike.%${t}%,isi_teks.ilike.%${t}%`)
            .join(",");
    ...
    if (categoryFilter !== "Semua") {
        ...
        if (keywords.length > 0) {
            ...
            const categoryString = keywords
                .map((kw: string) => `judul.ilike.%${kw}%,isi_teks.ilike.%${kw}%`)
                .join(",");
    ...
    if (isSearch) {
        ...
        const ranked = (data || [])
            .map((b: any) => ({ item: b, score: relevanceScore(b, searchQuery) }))
            .sort(
                (a: any, b: any) =>
                    b.score - a.score ||
        ...
        count = ranked.length;
        beritaData = ranked.slice(from, to + 1).map((r: any) => r.item);
    } else {
    ...
    let transformed = (beritaData || []).map((item: any) =>
