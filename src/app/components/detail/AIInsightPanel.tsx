...
export default function AIInsightPanel({ news, isLive = false }: AIInsightPanelProps) {
  const dominantSentiment: "Positif" | "Negatif" | "Netral" = (() => {
    const s = news.sentiments;
    if (!s?.length) return "Netral";
    return s.reduce((prev, curr) => curr.percentage > prev.percentage ? curr : prev).type;
  })();
  const config = sentimentConfig[dominantSentiment];
          ...
          {news.impacts && news.impacts.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {news.impacts.map((imp: { name: string; percentage: number }, i: number) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    <TagBadge text={imp.name} variant="impact" />
                  </motion.div>
                ))}
              </div>
            </div>
